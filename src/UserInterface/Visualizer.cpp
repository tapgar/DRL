...
void Visualizer::profilerUpdateForces( Matrix<double, nC*DOF, 1> f, Matrix<double, nU, 1> u)
{
	float dMax = -10000;
	float dMin = 10000;
	int pnt = mjMIN(201, figForce.linepnt[0]+1);
	for (int n = 0; n < nC*DOF; n++)
	{
		for (int i = pnt - 1; i > 0; i--)
		{
...
void Visualizer::profilerUpdateCOP(Matrix<double, DOF, 1> COM, Matrix<double, DOF, 1> COM_target, Matrix<double, DOF, nC> foot, bool* bContact)
{
	float dMaxX = -10000;
	float dMinX = 10000;
	float dMaxY = -10000;
...
void Visualizer::DrawMPCPlan(mjData* data) {
	for (int i = 0; i < plan_com_traj.size(); i++)
	{
		mjtNum pos[3];
	...
	for (int i = 0; i < plan_cop_traj.size(); i++)
	{
		mjtNum pos[3];
	...
	for (int i = 0; i < plan_foot_pos.size(); i++)
	{
		mjtNum pos[3];
